...
struct fat_directory_item
{
    ...
    uint8_t attribute;
    uint8_t reserved;
    uint8_t creation_time_thenths_of_a_sec;
...
struct fat_private
{
    ...
    // Used in situlations where we stream the directory
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int offset, int total, void *out)
{
    int res = 0;
    struct fat_private *private = disk->fs_private;
    int size_of_cluster_bytes = private->header.primary_header.bytes_per_sector * disk->sector_size;
