...
int fopen(const char *filename, const char *mode_str)
{
    ...
    // Check if it's just a root path 0:/
    if (!root_path->first)
    {
        res = -EINVARG;
        goto out;
...
int fstat(int fd, struct file_stat *stat)
{
    ...
    res = desc->filesystem->stat(desc->disk, desc->private_ptr, stat);
...
int fclose(int fd)
{
    ...
    res = desc->filesystem->close(desc->private_ptr);
...
int fseek(int fd, int offset, FILE_SEEK_MODE whence)
{
    ...
    res = desc->filesystem->seek(desc->private_ptr, offset, whence);
...
int fread(void *ptr, uint32_t size, uint32_t nmemb, int fd)
{
    ...
    res = desc->filesystem->read(desc->disk, desc->private_ptr, size, nmemb, (char *)ptr);
